...
def handle_data(row: pd.Series) -> dict[str, str]:
    """
    Process the data from a row of a DataFrame and return a dictionary containing the converted data.

    Args:
        row (pd.Series): A row of a DataFrame containing the data to be processed.

    Returns:
        dict[str, str]: A dictionary containing the converted data.
    ...
    days_ahead = (weekday_mapping.get(row["THỨ"], None) - start_day.weekday()) % 7
